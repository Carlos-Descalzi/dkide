...
import io.datakitchen.ide.dialogs.NewDataSourceDialog;
...
public class NewDataSourceAction extends AnAction {
    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
        ...
        if (project != null && folder != null) {
            ...
            NewDataSourceDialog dialog = new NewDataSourceDialog(module);
            if (dialog.showAndGet()) {
                dialog.writeToFolder(module, folder, false, (dsFile) -> {
                    ProjectView.getInstance(project).refresh();
                    ApplicationManager.getApplication().invokeLater(()->
                            FileEditorManager.getInstance(project).openFile(dsFile, true)
                    );
                });
            }
        }
        ...
        if (module != null && folder != null) {
            try {
                if (RecipeUtil.isNodeFolder(module, folder)) {
                    String dataSourceFolderName = RecipeUtil.getDataSourcesFolderNameForNode(folder);

                    return dataSourceFolderName != null && folder.findChild(dataSourceFolderName) == null;
                } else {
                    return RecipeUtil.isDataSourceFolder(module, folder);
                }
